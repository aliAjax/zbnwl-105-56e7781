...
import { ArrowLeft, Calendar, Clock, MapPin, DollarSign, FileText, User, CheckCircle, History, AlertCircle, TrendingUp, MinusCircle, Users, X, UserPlus, Edit2, Save } from 'lucide-react';
import { useAppointmentsRepository, useCustomerMergesRepository } from '@/storage';
import { buildCustomerProfile, buildAliasMap, getCanonicalName, getAllRawCustomerNames, getAliasesForCanonical, mergeCustomers, removeAlias, changeCanonicalName } from '@/utils/customerUtils';
...
export default function CustomerProfile() {
  ...
  const hasAnyData = profile !== null || aliases.length > 0;
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="font-display text-2xl font-bold text-white">{canonicalName}</h1>
                  <button
                    onClick={() => {
                      setNewCanonicalName(canonicalName);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        <div className="bg-ink-800/30 rounded-2xl border border-ink-700 p-5">
          ...
          {aliases.length > 0 ? (
            ...
            <p className="text-gray-500 text-sm">暂无别名，点击"合并客户"将多个昵称合并为同一客户</p>
          )}
        </div>
              ...
              <div className="space-y-3">
                {profile.history.map((apt) => (
                  ...
                  >
                    ...
                    {apt.customerName !== canonicalName && (
        ...
        {!profile && (
          <div className="bg-ink-800/30 rounded-2xl border border-ink-700 p-8 text-center">
            <AlertCircle className="w-12 h-12 text-gray-500 mx-auto mb-3" />
            <p className="text-gray-400">该客户暂无预约记录</p>
            <p className="text-gray-500 text-sm mt-1">可以继续管理客户别名，预约后将自动显示统计信息</p>
          </div>
        )}
      </div>
          ...
          <div className="relative bg-ink-800 rounded-2xl border border-ink-700 w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              <div className="bg-ink-900/50 border border-ink-700 rounded-xl p-4">
                ...
                <p className="text-white font-medium text-lg mt-1">{canonicalName}</p>
              </div>
              <div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  disabled={!newCanonicalName.trim() || newCanonicalName.trim() === canonicalName}
                  className={`flex-1 px-4 py-2.5 rounded-lg font-medium transition-colors flex items-center justify-center gap-2 ${
                    newCanonicalName.trim() && newCanonicalName.trim() !== canonicalName
                      ? 'bg-gold-500 hover:bg-gold-400 text-ink-950'
                      : 'bg-gray-600 text-gray-400 cursor-not-allowed'
                  }`}
                >
